...
audio_wakewords_wakenet_v_init(audio_wakewords_t *wakewords)
{
    ...
    for (size_t i = 0; i < srmodel_list->num; ++i) {
        ESP_LOGI(TAG, "Model %zu: name=%s, info=%s", i, srmodel_list->model_name[i],
...
#    ifdef CONFIG_AUDIO_WAKEWORD_WAKENET_AEC_ENABLED
    ...
    afe_config_t *afe_cfg = afe_config_init(input_format, srmodel_list, AFE_TYPE_SR,
                                            AFE_MODE_HIGH_PERF);
    ...
    // TODO: xTaskCreate for wakeword detection task, and call state->afe_iface->feed in
    // the task.
#    else
    ESP_LOGI(TAG,
             "Initializing WakeNet wakeword detection engine without AEC support.");
#    endif
#else
    ESP_LOGE(TAG,
             "WakeNet wakeword detection engine is not enabled in the configuration.");
